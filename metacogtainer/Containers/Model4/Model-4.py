...
# savetxt('/app/docker_bind/FA_cd_model_4.csv', FA_cd, delimiter=',')
...
for i in range(len(data_ss) if args.max_test is None else args.max_test):
    ...
    if max_disc == MODEL_NUMBER:
        ...
        temp = np.expand_dims(data_ss[i,:],0) # Called in run det
        model4_thresh = model_thresh4.predict(temp,verbose = 0) # Called in function 'one'

        model_4_thresholds.append( [i, model4_thresh] )

savetxt('/app/docker_bind/model_4_thresholds.csv', np.array(model_4_thresholds), delimiter=',')