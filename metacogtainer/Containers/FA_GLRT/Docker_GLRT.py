...
# NEW : Modified version of runDet to only return GLRT value
def runGLRT(data_ss, z, S,p, models, options, test_num, args):
    temp = np.expand_dims(data_ss[test_num,:],0)
    det_glrt = options[0](args,p,temp, z, S,models[0])
    return det_glrt
...
for i in range(len(data_ss) if args.max_test is None else args.max_test):
    ...
    det_glrt = runGLRT(data_ss, z_full[i],  S[i,:,:], p, models, options, i,args)
...
if args.show_output is True:
    print("Generalized Likelihood Ratio Test (GLRT):",FA_glrt[0])
