...
def execute_exp(args=None):
    ...
    if args.conv_size is None:
        ...
        test_x0=data[train_portion+val_portion:]
        test_y0=label[train_portion+val_portion:]
    ...
    if args.conv_size is None:
        if (args.lstm_size or args.gru_size) is None:
            ...
            model=create_mlp_thresh(data.shape[1], args.nclasses,
...
def display_Metrics(args1, test=False,save=False):    
    ...
    for i in range(np.size(test_label_ind,axis=1)):
        test_labels[i] = test_label[np.squeeze(test_label_ind[0,i])]
        test_preds[i] = test_pred[test_label_ind[0,i]]
        
    test_diff = test_labels-test_preds    
    ...
    plt.figure(2)
    plt.hist(test_diff,bins=50)
    
    # val_preds = np.argmax(val_pred, axis=1)
    # #Generate the confusion matrix 
    # conf_matrix3=sklearn.metrics.confusion_matrix(test_label, test_pred, sample_weight=None, normalize='true')
